...
RSpec.describe "Measurements", type: :request do
  ...
  describe "POST /measurements" do
    it "should create a new measurement" do
      ...
      expect { post '/api/v1/measurements', params: {
        measurement: {
  ...
  describe "GET /measurements/:id" do
    ...
    it "should return single measurement data" do
      get '/api/v1/measurements/' + measurement.id.to_s

      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body)).to include("name"=>"Test", "image"=>"http://placehold.it/500x500", "measures"=>[])
    end
  end
end
